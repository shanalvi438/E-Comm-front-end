...
const TabProduct: NextPage<TabProductProps> = ({ catId, effect, menus }) => {
    ...
    if (activeTab && subCategoriesData.length > 0) {
      ...
      if (activeSubCategory) {
        ...
        axios
          ...
          .then((res) => {
            console.log(
              `Fetched ${res.data.products.length} products from the backend.`
            );
  ...
  const next = () => {
    ...
    if (currentIndex === 6 && subCategoriesData.length > 7) {
      console.log(`Current Index: ${currentIndex}, Next Index: 9`);
      ...
      if (visibleCategories === 6) {
        console.log(
          `Visible Categories: ${visibleCategories}, Adjusting Active Index`
        );
        setActiveIndex(
  ...
  return (
    <div className="section-pt-space custom-container bg-white mt-2   ">
                ...
                {subCategoriesData.map((subCategory, i) => (
                  <CarouselItem key={subCategory.id}>
                    <ul className="product-catogories">
                      {subCategoriesData.slice(i, +5).map((subCategory) => (
      ...
      <section className="section-py-space ratio_asos product custom-container ">
                  ...
                  <div className="product product-slide-6 product-m no-arrow">
                    <div>
                      {loading ? (
                        ...
                        <Slider key={productsData.length} {...settings}>
                        ...
                        // Log the total number of products being displayed
                      )}
                    </div>
                  </div>
                </TabPane>
